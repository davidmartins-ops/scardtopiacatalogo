    ...
    <div className="space-y-6 overflow-visible">
      <div className="glass-card p-4 space-y-4 animate-fade-in-up overflow-visible" style={{ animationDelay: "0.2s", opacity: 0 }}>
        <div className="relative">
          <label htmlFor="catalog-search" className="sr-only">Buscar produtos por nome</label>
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" aria-hidden="true" />
          <Input id="catalog-search" type="search" aria-label="Buscar produtos por nome" placeholder="Buscar por nome..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10 bg-muted/30 border-border/50 backdrop-blur-sm focus:border-primary/50 transition-colors" />
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Input type="number" aria-label="Preço mínimo em reais" placeholder="Mín" value={priceMin} onChange={(e) => setPriceMin(e.target.value)} className="w-24 h-8 text-xs bg-muted/30 border-border/50" min="0" />
          <span className="text-xs text-muted-foreground" aria-hidden="true">—</span>
          <Input type="number" aria-label="Preço máximo em reais" placeholder="Máx" value={priceMax} onChange={(e) => setPriceMax(e.target.value)} className="w-24 h-8 text-xs bg-muted/30 border-border/50" min="0" />
