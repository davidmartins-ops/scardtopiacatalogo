...
const ItemGrid = ({ items }: { items: ReturnType<typeof useInventory>["data"] }) => {
    ...
    <div className="space-y-6">
      ...
      <div className="glass-card p-4 space-y-4 animate-fade-in-up" style={{ animationDelay: "0.2s", opacity: 0 }}>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Badge variant={activeCategory === null ? "default" : "outline"} className="cursor-pointer transition-all duration-200 hover:scale-105" onClick={() => setActiveCategory(null)}>
            Todos
          </Badge>
          {categories.map((cat) => (
            <Badge key={cat} variant={activeCategory === cat ? "default" : "outline"} className="cursor-pointer transition-all duration-200 hover:scale-105" onClick={() => setActiveCategory(activeCategory === cat ? null : cat)}>
              {cat}
            </Badge>
          ))}
        </div>
                      ...
                      <div className="overflow-hidden rounded-xl border border-border/40 bg-muted/20 relative">
                        {item.image_url ? (
                          <ImageZoom
                            src={item.image_url}
                            alt={item.name}
                            className={`w-full h-44 sm:h-48 object-cover transition-transform duration-500 ${isOutOfStock ? "grayscale" : ""}`}
                        ...
                        <div className="text-right">
                          {discount > 0 ? (
                            <>
                              ...
                              <Badge variant="outline" className="ml-2 text-[10px] bg-accent/15 text-accent border-accent/30">
                                -{discount}%
                              </Badge>
                            </>
                          ) : (
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 -mt-4 relative z-20 pb-12">
        <Tabs defaultValue="drops" className="w-full">
          ...
          <TabsContent value="drops">
            <ItemGrid items={drops} />
          </TabsContent>

          <TabsContent value="singles">
            <ItemGrid items={singles} />
          </TabsContent>
        </Tabs>
      </div>
